...
from mava.custom_env.multiwalker.jaxmarl import MultiWalkerEnv, StateWithStep
...
class MultiWalkerWrapper(JaxMarlWrapper):
    """Wrraper for the MultiWalker environment."""
    ...
    def __init__(
        ...
        super().__init__(env, has_global_state, 500)
    ...
    def action_mask(self, wrapped_env_state: StateWithStep) -> Array:
        """Get action mask for each agent."""
        return jnp.ones((self.num_agents, self.action_dim), dtype=bool)

    def get_global_state(self, wrapped_env_state: StateWithStep, obs: Dict[str, Array]) -> Array:
        """Get global state from observation and copy it for each agent."""
        global_obs = self._env.get_world_obs(wrapped_env_state, obs)
